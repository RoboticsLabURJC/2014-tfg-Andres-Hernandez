...
class Tran37(TemporalTransition):
...
class Interfaces():
	def __init__(self):
		...
		self.timeLimitToChange = 10000 #In seconds
	...
	def connectProxies(self):
		...
		self.jdrc = comm.init(cfg, "performance_measurer")
		self.myCmdvel = self.jdrc.getCMDVelClient("performance_measurer.myCmdvel")
		if not self.myCmdvel:
			raise Exception("could not create client with name:myCmdvel")
		print("myCmdvel is connected")
		self.myCamera = self.jdrc.getCameraClient("performance_measurer.myCamera")
		if not self.myCamera:
			raise Exception("could not create client with name:myCamera")
		print("myCamera is connected")
		self.myExtra = self.jdrc.getArDroneExtraClient("performance_measurer.myExtra")
		if not self.myExtra:
			raise Exception("could not create client with name:myExtra")
		print("myExtra is connected")
		self.myPose = self.jdrc.getPose3dClient("performance_measurer.myPose")
		if not self.myPose:
...
if __name__ == "__main__":
	...
	if displayGui:
		...
		gui.addState(20, "Initialization", True, 846.0, 733.0, 0)
		...
		gui.addTransition(37, "transition 37", 20, 1, 1050.5, 862.0)
	...
	tran37 = Tran37(37, 1, 500)
	state20.addTransition(tran37)
