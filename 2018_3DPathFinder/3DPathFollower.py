...
class Tran28(ConditionalTransition):
	...
	def runCode(self):
		logging.basicConfig(filename='april_performance',format='%(asctime)s %(message)s',level=logging.INFO)
...
class Tran14(ConditionalTransition):
	...
	def runCode(self):
		logging.basicConfig(filename='color1_performance',format='%(asctime)s %(message)s',level=logging.INFO)
		logging.info(str(self.interfaces.minf) + ";" + str(self.interfaces.avgf) +";" + str(self.interfaces.maxf))
		self.interfaces.myExtra.toggleCam()
		input_image = self.interfaces.myCamera.getImage()
...
class Tran24(ConditionalTransition):
	...
	def runCode(self):
		logging.basicConfig(filename='color2_performance',format='%(asctime)s %(message)s',level=logging.INFO)
		logging.info(str(self.interfaces.minf) + ";" + str(self.interfaces.avgf) +";" + str(self.interfaces.maxf))

class Tran36(TemporalTransition):
...
class Interfaces():
	...
	def connectProxies(self):
		...
		self.myPose = self.jdrc.getPose3dClient("escenario_combinado.myPose")
		if not self.myPose:
...
if __name__ == "__main__":
	...
	if displayGui:
		...
		gui.addState(20, "Initialization", False, 846.0, 732.0, 0)
		gui.addState(21, "Spiral Search", False, 1158.0, 971.0, 0)
		gui.addState(23, "Color Beacon Landing", False, 1154.0, 1089.0, 0)
		gui.addState(24, "Take off", True, 1107.0, 729.0, 0)
		...
		gui.addTransition(36, "Taking off", 24, 20, 984.0, 731.0)

	if displayGui:
		gui.emitLoadFromRoot()
		gui.emitActiveStateById(0)

	state0 = State0(0, True, interfaces, 40, None, gui)
	...
	tran36 = Tran36(36, 20, 100)
	state24.addTransition(tran36)
