		    ...
		    if(len(positionXarr)>0):
		        if(positionXarr[0] != -20 and positionYarr[0]!=-20):
		            ...
		            if(abs(yError)<25):
		                ...
		                vy= yError*self.interfaces.kp #0.01
		                
		            if(abs(xError)<25):
		                print "Vy en banda de seguridad"
		                vx=0
		            else:
		                vx= xError*self.interfaces.kp #0.01
...
class Interfaces():
	...
	def connectProxies(self):
		...
		self.jdrc = comm.init(cfg, "precise_landing_simulated")
		self.myCamera = self.jdrc.getCameraClient("precise_landing_simulated.myCamera")
		if not self.myCamera:
			raise Exception("could not create client with name:myCamera")
		print("myCamera is connected")
		self.myPose3D = self.jdrc.getPose3dClient("precise_landing_simulated.myPose3D")
		if not self.myPose3D:
			raise Exception("could not create client with name:myPose3D")
		print("myPose3D is connected")
		self.myCMDVel = self.jdrc.getCMDVelClient("precise_landing_simulated.myCMDVel")
		if not self.myCMDVel:
			raise Exception("could not create client with name:myCMDVel")
		print("myCMDVel is connected")
		self.myNavData = self.jdrc.getNavdataClient("precise_landing_simulated.myNavData")
		if not self.myNavData:
			raise Exception("could not create client with name:myNavData")
		print("myNavData is connected")
		self.myDroneExtra = self.jdrc.getArDroneExtraClient("precise_landing_simulated.myDroneExtra")
...
if __name__ == "__main__":
	...
	if displayGui:
		...
		gui.addState(1, "Takeoff_simulated", True, 827.0, 939.0, 0)
		gui.addState(2, "Precise Landing", False, 974.0, 780.0, 0)
		gui.addState(3, "Landed", False, 1146.0, 939.0, 0)

		gui.addTransition(5, "transition 5", 1, 2, 824.0, 782.5)
	...
	state0 = State0(0, True, interfaces, 300, None, gui)
	...
	tran5 = Tran5(5, 2, 15000)
	state1.addTransition(tran5)
